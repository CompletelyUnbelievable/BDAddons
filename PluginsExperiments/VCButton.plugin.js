...
class VCButton{
	...
	observer({addedNodes}){//I'm not sure how to do this any better.
		if(addedNodes.length===1&&addedNodes[0]&&addedNodes[0].classList&&addedNodes[0].classList.contains(this.classes.channelContainer)){
			this.init();
		}
	}

	init(){
		this.addButtonsToElement(this.cleanArray(this.getCurrentChannelsByType(2).map((v)=>{return this.channelIdToElement(v.id);})));
	}
		...
		if(elements&&this.toType(elements)==='array'){
			...
			iconEles.forEach(function(ele,index){
				if(!ele.getElementsByClassName(this.getName())[0]){
					...
					template.addEventListener('click',function(){if(this.changeChannel&&reactEles&&reactEles[index]&&reactEles[index].guild_id&&reactEles[index].id)this.changeChannel(reactEles[index].guild_id,reactEles[index].id);}.bind(this));
