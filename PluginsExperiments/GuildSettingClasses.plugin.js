...
class GuildSettingClasses{
	...
	start(){
		this.classes=["GSCMyAccount","GSCAuthorizedApps","GSCConnections","GSCBilling","GSCDiscordNitro","GSCHypeSquad","GSCVoiceVideo","GSCOverlay","GSCNotifications","GSCKeybinds","GSCGameActivity","GSCGameLibrary","GSCTextImages","GSCAppearance","GSCStreamerMode","GSCLanguage","GSCWindowsSettings","GSCChangeLog","GSCLogOut","GSCCore","GSCZeresFork","GSCBandages","GSCEmotes","GSCCustomCSS","GSCPlugins","GSCThemes"];
		BdApi.injectCSS(`${this.getName()}CSS`,this.GSCsetStyles());
		this.observer();
	}

	observer(){
		let settingsItems=['item-PXvHYJ','ui-tab-bar-item'];
		for(let i of settingsItems){
			if(document.getElementsByClassName(i)[0])this.GSCFindModify(i);
		}
	}

	stop(){
		if(document.getElementById(`${this.getName()}CSS`))BdApi.clearCSS(`${this.getName()}CSS`);
		this.GSCRemove();
	}

	GSCClassList(x){	
		if(!Array.isArray(x)&&typeof x==='string'&&!this.classes.includes(x)){
			this.classes.push(x);
		}else if(Array.isArray(x)){
			for(let y of x){
				if(typeof y==='string'&&!this.classes.includes(y)){
					this.classes.push(y);
				}
			}
		}
	...
	GSCFindModify(x){
		for(let element of document.getElementsByClassName(x)){
			let soloClass=`GSC${element.innerHTML.replace(/ |'|&amp;|&/igm,"")}`;
			if(!element.classList.contains(soloClass)){
				element.classList.add(soloClass);
				this.GSCClassList(soloClass);
			}
		}
	}
	...
	GSCRemove(){
		if(document.getElementsByClassName('ui-standard-sidebar-view')[0]||document.getElementsByClassName('ui-tab-bar-item')[0]){
			for(let i of this.classes){
				for(let x of document.getElementsByClassName(i)){
	...
	GSCsetStyles(){
		let styles=
		`#user-settings .item-PXvHYJ,#bd-settings-sidebar .ui-tab-bar-item{
			padding-left: 34px;
		}
		...
		#user-settings .item-PXvHYJ.GSCActivityFeed::before{
			-webkit-mask: var(--DefaultActivitySVG);
		}
